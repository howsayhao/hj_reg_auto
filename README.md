...
- `addrmap`: similar to `regfile` on packing register and allocating addresses. What's different and more important, `addrmap` defines the **RTL code generation boundary**. There are four types of `addrmap` which corresponds to `regmst`, `regdisp`, `regslv` and `3rd party IP` in the RTL architecture respectively, and they are distinguished by several user-defined properties in HRDA
...
Each SystemRDL `field` instance will be generated to a `field` module instance inside `regslv`. In generated RTL, stem name of field is `<reg_inst_name>__<field_inst_name>`. Other signals belong to the field are named by prefixing/suffixing elements.
...
<div style="display: inline-block;
color: #999;
padding: 5px;">Table 3.4 supported regfile component properties</div>
</center>
...
Memory definition accepts properties listed in [Table 3.5](#table_mem_prop).
...
<div style="display: inline-block;
color: #999;
padding: 5px;">Table 3.5 supported memory component properties</div>
</center>

If *memwidth* is larger than *accesswidth* and each memory entry occupies $N$ address slots where $N$ should be power of 2 ($2^i$) to simplify decode logic, bridge units between `regdisp` and memory interface will implement a *snapshot register* to atomically read/write memory entries.
...
3. `addrmap` instances are handled in different ways depending on the property assignment of `hj_genslv` and `hj_flatten_addrmap` in `addrmap` definition. Detailed configuration is listed in [Table 3.6](#table_addrmap_recog)

// FIXME
<span id="table_addrmap_recog"></span>
| hj_genslv | hj_flatten_addrmap | handling behavior | Usage |
|-----------|--------------------|-------------------|-------|
...
<div style="display: inline-block;
color: #999;
padding: 5px;">Table 3.6 properties to recognize different type of addrmap in RTL</div>
</center>

Other suppored properties for `addrmap` is listed in [Table 3.7](#table_addrmap_prop).
...
```systemrdl
addrmap some_map { // Define a Bridge Device
  desc="xxx";
  reg status {
